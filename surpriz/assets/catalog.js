(() => {
  ...
  const SYNONYM_GROUPS = [
    ...
    ["ледибаг", "ladybug", "miraculous", "леди баг", "супер кот", "cat noir"],
  ...
  const buildQueryContext = (query) => {
    ...
    return {
      normalizedQuery,
      compactQuery: normalizedQuery.replace(/ /g, ""),
      variants: [...expandQueryVariants(query)],
    };
  };

  const searchScore = (character, { normalizedQuery, compactQuery, variants }) => {
  ...
  const applyCrop = (image, item, defaultFit) => {
    const position = item.image_position || {};
    const x = clampNumber(position.x, 50, 0, 100);
    const y = clampNumber(position.y, 50, 0, 100);
    const fit = readFit(item.cover_fit, defaultFit);
    image.style.objectFit = fit;
    image.style.objectPosition = `${x}% ${y}%`;
    image.style.transformOrigin = `${x}% ${y}%`;
    image.style.setProperty("--image-zoom", clampNumber(item.image_zoom, 100, 100, 200) / 100);

    const mobilePosition = item.mobile_image_position || position;
    image.style.setProperty("--mobile-image-x", `${clampNumber(mobilePosition.x, x, 0, 100)}%`);
    image.style.setProperty("--mobile-image-y", `${clampNumber(mobilePosition.y, y, 0, 100)}%`);
    image.style.setProperty("--mobile-image-fit", readFit(item.mobile_cover_fit, fit));
    image.style.setProperty(
      "--mobile-image-zoom",
      clampNumber(item.mobile_image_zoom, clampNumber(item.image_zoom, 100, 100, 200), 100, 200) / 100,
    );
  };
  ...
  const makeCharacterSource = (base, format, sizes) => {
    ...
    source.srcset = [480, 768, 1200].map((width) => `${base}-${width}.${format}?v=b970d6020c37 ${width}w`).join(", ");
    source.sizes = sizes;
    return source;
  };
  ...
  const createCharacterCard = (character, index) => {
    ...
    // Фото — такая же ссылка на карточку, как название: посетитель первым делом
    // жмёт по картинке, а не по подписи под ней.
    const detailHref = String(character.detail_href || toPageHref(character.href));
    const media = document.createElement("a");
    media.href = detailHref;
    media.setAttribute("aria-label", `Подробнее: ${character.title || "персонаж"}`);
    ...
    if (imageSrc) {
      ...
      image.width = 800;
      image.height = 1200;
      applyCrop(image, character, "contain");
  ...
  const searchedCharacters = () =>
    characters.filter((character) => matchesSearch(character, searchQuery));

  const renderGrid = (searched = searchedCharacters()) => {
    const query = searchQuery.trim();
    let visible = searched.filter((character) => matchesCategory(character, activeFilter));
    if (query) {
      const context = buildQueryContext(searchQuery);
      visible = visible
        .map((character) => [searchScore(character, context), character])
        .sort((left, right) => left[0] - right[0])
        .map(([, character]) => character);
    }
    grid.replaceChildren(...visible.map(createCharacterCard));
    if (emptyMessage) emptyMessage.hidden = Boolean(query) || visible.length > 0;
    if (searchStatus) {
  ...
  const updateFilters = (searched = searchedCharacters()) => {
  ...
  const updateMenuState = () => {
    document.querySelectorAll("[data-character-filter]").forEach((link) => {
      if (link.dataset.characterFilter === activeFilter) link.setAttribute("aria-current", "page");
  ...
  const applyFilter = (value, { updateUrl = true } = {}) => {
    ...
    const searched = searchedCharacters();
    updateFilters(searched);
    renderGrid(searched);
    if (updateUrl) {
